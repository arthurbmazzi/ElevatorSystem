...
builder.Services.AddSwaggerGen(options =>
{
    ...
    {
        Title = "Elevator System API", Version = "v1",
        Description = "Submit trips; elevators process them automatically in FIFO order. " +
...
builder.Services.AddHostedService<ElevatorWorker>();
...
app.MapPost("/requests", async (CreateTripCommand command, FleetSession session, CancellationToken ct) =>
{
...
}).WithTags("1. Trips").WithSummary("Submit a trip for automatic processing")
    .Produces<TripSnapshot>(201).ProducesProblem(400).ProducesProblem(403).ProducesProblem(409);

app.MapGet("/trips", (FleetSession session, CancellationToken ct) =>
    session.ExecuteAsync(s => s.Trips, ct)).WithTags("1. Trips").WithSummary("View trips and their states");
app.MapGet("/trips/{id:guid}", (Guid id, FleetSession session, CancellationToken ct) =>
...
app.MapGet("/elevators", (FleetSession session, CancellationToken ct) =>
...
app.MapGet("/analytics", (FleetSession session, CancellationToken ct) =>
    session.ExecuteAsync(s => s.GetAnalytics(), ct)).WithTags("3. Monitoring").WithSummary("View totals and metrics");
app.MapGet("/events", (FleetSession session, CancellationToken ct) =>
    session.ExecuteAsync(s => s.Events, ct)).WithTags("3. Monitoring").WithSummary("View the latest 1,000 events");
app.MapGet("/logs/status", (FileLogProvider logs) => logs.GetStatus())
    .WithTags("3. Monitoring").WithSummary("Find the active TXT file and check write or retention errors");

app.Run();

void MapCarCommand(string route, string summary, Action<EnterpriseElevatorSystem, int> action)
{
    app.MapPost($"/elevators/{{id:int}}/{route}", (int id, FleetSession session, CancellationToken ct) =>
